...
class StreamingChatTTS:
    def __init__(self):
        repo_dir = snapshot_download("pengzhendong/ChatTTS")
        self.gpt = GPT(
            {
                "hidden_size": 768,
            ...
            f"{repo_dir}/Embed.safetensors",
            f"{repo_dir}/gpt",
            f"{repo_dir}/tokenizer",
            device="cpu",
        )
        self.sdvae = StreamingDVAE()
        self.normalizer = Normalizer(lang="zh", operator="tn")

    def generate(self, text):
        text = self.normalizer.normalize(text)
        for tokens in self.gpt.generate(text):
